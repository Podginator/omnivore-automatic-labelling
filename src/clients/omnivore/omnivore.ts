...
export class OmnivoreClient {
  ...
  async setLabels(pageId: string, labels: Label[]): Promise<void> {
    ...
    await axios.post(
      `${API_URL}/graphql`,
      { query: mutation, variables: { input: { pageId, labelIds } } },
      {
        headers: {
          Cookie: `auth=${this.token};`,
          'Content-Type': 'application/json',
...
export const omnivoreClient: Promise<OmnivoreClient> = (async () => {
  return await OmnivoreClient.createOmnivoreClient(
